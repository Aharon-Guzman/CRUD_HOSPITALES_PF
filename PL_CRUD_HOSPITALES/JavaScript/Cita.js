...
$(document).ready(function () {
    ...
    if (pageName == "frmConsultaTiposCita.aspx") {
        cargaListaTiposCita();
    }
    else if (pageName == "frmMantenimientoTiposCita.aspx") {
        obtieneDetalleTipoCita();
    }
});

function crearTipoCita() {
    ...
    $.cookie("CITUNI", 0, { expires: TLTC, path: '/', domain: g_Dominio });
    location.href = "frmMantenimientoTiposCita.aspx";
}

function regresar() {
    location.href = "frmConsultaTiposCita.aspx";
}


function cargaListaTiposCita() {
    //nombre, valor, [expiracion, path, domain]
    $.cookie("CITUNI", 0, { expires: TLTC, path: '/', domain: g_Dominio });

    //Crear un objeto para almacenar la información del formulario
    var obj_Parametros_JS = new Array();
    obj_Parametros_JS[0] = $("#bsqTipo_Cita").val();
    ...
    if ((obj_Parametros_JS[2] != 0) && (obj_Parametros_JS[2] != undefined)) {
        jQuery.ajax({
            type: "POST",
            url: "frmConsultaTiposCita.aspx/CargaListaTiposCita",
            data: parametros,
            ...
            success: function (msg) {
                ...
                if (res === undefined) {
                    ...
                    if (res === "No se encontraron registros") {
                        ...
                        $("#tblTiposCita").html(res);
                        paginar("#tblTiposCita");
                    }
                }
            },
...
function defineTipoCita(uni) {
    $.cookie("CITUNI", uni, { expires: TLTC, path: '/', domain: g_Dominio });
    location.href = "frmMantenimientoTiposCita.aspx";
}

function obtieneDetalleTipoCita() {
    //Crear un objeto para almacenar la información del formulario
    var obj_Parametros_JS = new Array();
    obj_Parametros_JS[0] = $.cookie("CITUNI");
    ...
    if ((obj_Parametros_JS[1] != 0) && (obj_Parametros_JS[1] != undefined)) {
        jQuery.ajax({
            type: "POST",
            url: "frmMantenimientoTiposCita.aspx/CargaInfoTipoCita",
            data: parametros,
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            cache: false,
...
function mantenimientoTipoCita() {
    //Crear un objeto para almacenar la información del formulario
    var obj_Parametros_JS = new Array();
    obj_Parametros_JS[0] = $.cookie("CITUNI");
    obj_Parametros_JS[1] = $("#txtTipo_Cita").val();
    ...
    if ((obj_Parametros_JS[3] != 0) && (obj_Parametros_JS[3] != undefined)) {
        jQuery.ajax({
            type: "POST",
            url: "frmMantenimientoTiposCita.aspx/MantenimientoTipoCita",
            data: parametros,
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            cache: false,
...
function eliminaTipoCita(uni) {
    ...
    if ((obj_Parametros_JS[1] != 0) && (obj_Parametros_JS[1] != undefined)) {
        jQuery.ajax({
            type: "POST",
            url: "frmMantenimientoTiposCita.aspx/EliminarTiposCita",
            data: parametros,
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            cache: false,
